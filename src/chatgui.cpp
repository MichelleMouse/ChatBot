...
{
  //Creates window with name and show it
  ChatBotFrame *chatBotFrame = new ChatBotFrame(wxT("Memory Management ChatBot"));
...
{
  //Creates panel with background image
  ChatBotFrameImagePanel *ctrlPanel = new ChatBotFrameImagePanel(this);

  //Creates controls and assign them to control panel
  _panelDialog = new ChatBotPanelDialog(ctrlPanel, wxID_ANY);

  //Creates text control for user input
  int idTextXtrl = 1;
  ...
  //Creates vertical sizer for panel alignment and add panels
  ...
  //Positions window in screen center
  this->Centre();
}

void ChatBotFrame::OnEnter(wxCommandEvent &WXUNUSED(event))
{
  //Retrieves text from text control
  wxString userText = _userTextCtrl->GetLineText(0);

  //Adds new user text to dialog
  _panelDialog->AddDialogItem(userText, true);

  //Deletes text in text control
  _userTextCtrl->Clear();

  //Sends user text to chatbot
...
EVT_PAINT(ChatBotFrameImagePanel::paintEvent) //Catches paint events
END_EVENT_TABLE()
...
{
  //Loads backgroud image from file
  wxString imgFile = imgBasePath + "sf_bridge.jpg";
  wxImage image;
  image.LoadFile(imgFile);

  //Rescales image to fit window dimensions
...
EVT_PAINT(ChatBotPanelDialog::paintEvent) //Catches paint events
END_EVENT_TABLE()
...
: wxScrolledWindow(parent, id)
{
  ...
  //Allows for PNG images to be handled
  wxInitAllImageHandlers();

  _chatLogic = std::make_unique<ChatLogic>();                             //Creates a ChatLogic instance
  _chatLogic->SetPanelDialogHandle(this);                                 //Passes pointer to chatbot dialog so answers can be displayed in GUI
  _chatLogic->LoadAnswerGraphFromFile(dataPath + "src/answergraph.txt");  //Loads answer graph from file
}
...
{
  //Adds a single dialog element to the sizer
  ...
  //Makes scrollbar show up
  this->FitInside(); // ask the sizer about the needed size
  this->SetScrollRate(5, 5);
  this->Layout();

  //Scrolls to bottom to show newest element
  int dx, dy;
...
{
  //Converts string into wxString and add dialog element
...
: wxPanel(parent, -1, wxPoint(-1, -1), wxSize(-1, -1), wxBORDER_NONE)
{
  ...
  //Creates sizer and add elements
